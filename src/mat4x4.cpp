...
mat4x4 makeRotationY(float fAngleRad) {
    mat4x4 matrix;
    matrix.m[0][0] =  cosf(fAngleRad * 0.5f);
    matrix.m[0][2] =  sinf(fAngleRad * 0.5f);
    matrix.m[1][1] =  1.0f;
    matrix.m[2][0] = -sinf(fAngleRad * 0.5f);
    matrix.m[2][2] =  cosf(fAngleRad * 0.5f);
    matrix.m[3][3] =  1.0f;
    return matrix;
}
...
mat4x4 pointAt(Vec3D &pos, Vec3D &target, Vec3D &up) {
    ...
    Vec3D a = newForward * up.dotProduct(newForward); // Vector_Mul(newForward, Vector_DotProduct(up, newForward));
    Vec3D newUp = up - a; // Vector_Sub(up, a);
    newUp.normalize();

    // New Right direction is easy, its just cross product
    Vec3D newRight = newUp.crossProduct(newForward); // Vector_CrossProduct(newUp, newForward);
